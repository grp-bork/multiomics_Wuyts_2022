...
scatterplots <-
  ...
  filter(x_omic != y_omic) %>% 
  filter(y_omic != "amplicon_sequencing") %>% 
  filter(x_omic != "metaproteomics") %>% 
  filter(!(y_omic == "metagenomics" & x_omic == "metatranscriptomics")) %>% 
  mutate(plot = map2(x_omic, y_omic, ~generate_scatter_plot(.x, .y, log = T)))
